...
def get_post(db = Depends(get_db), current_user: int = Depends(oauth2.get_current_user),
    ...
    result = db.query(postmodel.Post, func.count(postmodel.Vote.post_id).label('Votes')).join(postmodel.Vote, 
                    postmodel.Vote.post_id == postmodel.Post.id, 
                    isouter=True).group_by(postmodel.Post.id).filter(postmodel.Post.user_id == current_user.id).filter(
                    postmodel.Post.title.contains(search)).limit(limit).offset(skip).all()
    # print result only can print when remove all() from the query above 
    # print(result)
    return result


# get post by id
@router.get("/{id}", response_model=schemas.PostResponseExt)
def get_post(id: int, db = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    post_item = db.query(postmodel.Post, func.count(postmodel.Vote.post_id).label('Votes')).join(postmodel.Vote, 
                    postmodel.Vote.post_id == postmodel.Post.id, 
                    isouter=True).group_by(postmodel.Post.id).filter(
                    postmodel.Post.user_id == current_user.id).filter(
                    postmodel.Post.id == id).first()
    if not post_item:
