...
def get_post(db = Depends(get_db), current_user: int = Depends(oauth2.get_current_user),
    ...
    post_query = db.query(postmodel.Post).filter(postmodel.Post.user_id == current_user.id).filter(postmodel.Post.title.contains(search)).limit(limit).Skip(skip).all()
    return post_query
